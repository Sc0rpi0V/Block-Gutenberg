...
class PostSelector extends Component {
	 ...
	 * saveLowPostData <Boolean> Save only post id, post title, post type
	 *
	 */
	constructor() {
		super(...arguments)
		...
		this.saveLowPostData = this.props.saveLowPostData ? this.props.saveLowPostData : false
	...
	componentDidMount(){
		...
		let type = wp.data.select('core/editor').getCurrentPost().type
		...
		const initialRequest = apiFetch({
			path: addQueryArgs('/wp/v2/' + type +'/' + post_id)
		})
	...
	async updateSuggestions(value) {
		...
		const request = apiFetch({
			path: addQueryArgs('/wp/v2/search?&embed&lang=' + this.state.lang, {
				search: value,
				per_page: 20,
				type: 'post',
				subtype: this.props.postType ? this.props.postType : null
			})
		})
	...
	onKeyDown(event) {
		...
		const { showSuggestions, selectedSuggestion, posts, loading, lang } = this.state
	...
	selectLink(post) {
		...
		apiFetch({
			...
			if(this.saveLowPostData) {
				this.props.onPostSelect({
					id: response.id,
					title: decodeEntities(response.title.rendered),
					type: response.type,
				})
	...
	renderSelectedPosts() {
		...
		if(this.props.limit == 1){
			...
			if(!_.isEmpty(post)) {
				return (
					<div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', flexWrap: 'nowrap' }} key={post.id}>
	...
	render() {
		...
		const inputDisabled = !!limit && (this.props.posts.length == limit || !_.isEmpty(this.props.posts))
			...
			<Fragment>
				...
				<div className="block-editor-url-input">
					<input
						...
						placeholder={inputDisabled ? sprintf(__(`Limité à %s élément(s)`, 'Gutenberg-back' ),  limit): __('Entrer le nom de votre publication', 'Gutenberg-back')}
				...
				!!posts.length && (
					...
					>
						<div className="block-editor-url-input__suggestions" style={{'width':'250px !important'}}  id={`block-editor-url-input-suggestions-${instanceId}`} ref={this.bindListNode} role="listbox">
...
export default withSelect((select) => {
	...
	return {
		sourcePostTypes: getPostTypes({ per_page: -1 })// in order to get more than 10 custom post type
	}
